...
class projectongoController extends Controller
{
   public function __construct ()
    ...
    {   
    	...
    	return view('project.projectOnGoII',[ 'data_group'    =>$data_group, 
                                    'data_menu'     =>$data_menu,
    ...
    public function pmo_project($projectType)
    {
        $data = new  pmo_project();
        $return = $data::where('summary',1)
                    ->where('projecttype_id',$projectType)
                    ->where('project_pid',0)
                    ->where('status',1)
                    ->orderBy('sort', 'asc')->get();
        return $return;
    }
    public function pmo_projectChild($project_id)
    {   
        $data = new  pmo_project();
        $return = $data::where('summary',1)
